...
def _write_valid_splits(data_dir: Path) -> None:
...
def _kernel_config(data_dir: Path, adapter_dir: Path) -> KernelRunConfig:
...
def test_validate_kernel_inputs_reports_empty_prompt_or_completion(
    ...
    adapter_dir = tmp_path / "adapter"
    _write_valid_splits(data_dir)
    _write_jsonl(data_dir / "train.jsonl", [{"prompt": "", "completion": "c"}])
    _write_adapter(adapter_dir)
